...
public class MonitoringUtil {
    ...
    public void processRawData(int timeStep, String rawData, MonitoringStatusDTO monitoringStatusDTO) {
    ...
    // 아기 상태 판단 todo 임시로 시리얼 번호 추가함 신규 매트 테스트용
    public MonitoringStatusDTO babyConditionJudgment(MonitoringStatusDTO matNode) {
        final int postureCode = matNode.postureStatus;
        final int breathCode = matNode.breathStatus;
        final int prevBabyState = matNode.babyState;

        String babyConditionJudgmentResult;
        ...
        // 자세 카운트
        int postureNotCenterCnt = matNode.postureNotCenterCnt; // 자세 좌/우
        int postureReverseCnt = matNode.postureReverseCnt; // 자세 뒤집힘
        int postureUnknownCnt = matNode.postureUnknownCnt; // 자세 측정불가

        // 호흡 카운트
        int breathLowCnt = matNode.breathLowCnt; // 호흡 낮음
        int breathVeryLowCnt = matNode.breathVeryLowCnt; // 호흡 매우 낮음
        ...
        if (POSTURE_STATUS_GOOD == postureCode) {
            ...
            if (DELAY_TIME_03 == postureNotCenterCnt && matNode.prevPostureStatus != POSTURE_STATUS_LEFT && matNode.prevPostureStatus != POSTURE_STATUS_RIGHT) {
        ...
        // 값 갱신
        matNode.isSendNotifications = isSendNotifications; // 알람 발송여부
        matNode.notificationCode = notificationCode; // 알람 발송 코드
        matNode.babyState = babyState; // 현재 아기 상태 응급/위험/좋음

        matNode.postureNotCenterCnt = postureNotCenterCnt; // 자세 좌/우 카운트
        matNode.postureReverseCnt = postureReverseCnt; // 자세 뒤집힘 카운트
        matNode.postureUnknownCnt = postureUnknownCnt; // 자세 측정불가 카운트

        matNode.breathLowCnt = breathLowCnt; // 호흡 낮음 카운트
        matNode.breathVeryLowCnt = breathVeryLowCnt; // 호흡 매우 낮음 카운트
